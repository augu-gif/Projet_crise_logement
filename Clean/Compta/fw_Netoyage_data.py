...
fb_fnb_cols = ["B31","B32","B33","B11TAFNB","B12TAFNB","B13TAFNB","B31TAFNB","B32TAFNB","B33TAFNB","WCOMFB","WCOMFNB","WEPCIFB","WEPCIFNB"]
df_rei_fb_fnb = df_rei[id_cols + fb_fnb_cols] # france entière"""
df_rei_fb_fnb_mel = df_rei[df_rei["Q03"] == "Métropole Européenne de Lille"] # MEL uniquement
#-----------------------------------------------------------------------------------------------------------------------------------------

#-----------------------------------------------------------------------------------------------------------------------------------------
# df_balance garder que les lignes de la MEL afin de se concentrer sur les données comptables des communes de la MEL
#-----------------------------------------------------------------------------------------------------------------------------------------
df_balance["NDEPT"] = df_balance["NDEPT"].astype(str).str.zfill(3) # s'assurer que les codes département sont au format 3 chiffres
df_mel["DEP"] = df_mel["DEP"].astype(str).str.zfill(3) # s'assurer que les codes département sont au format 3 chiffres
df_balance_mel = df_mel.merge(
    df_balance,
    left_on=["DEP", "COM"],
    right_on=["NDEPT", "INSEE"],
    how="left"
)
#-----------------------------------------------------------------------------------------------------------------------------------------
